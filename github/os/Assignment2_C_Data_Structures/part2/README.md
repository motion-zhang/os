...
Our stack that we are implementing can expand dynamically, thus a linked list is a good foundational data struturre to use. In our  
...
Stack concept come from Alan Turing in 1946. Stack is useful because it is used in lots of computer applications. For example, reverse a word and undo. Stack is also used in language processing, recursions, designning of interpreters...

One interesting thing is stack is used in expression evualtion, which related to complexity. A "Prefix evaluation Algorithm" using stack can improve the complexity to O(n), using pop(), push(). So as we can conclude, stack data structure can improve the run time of algorithm. 
