...
Ring buffers are compact, efficient and easy to implement. It is used in many areas. For examples, it is used in bounded logging, I/O buffering, realtime processing. 
It reuses the memory, so as one chunk of memory is processed, it becomes available for the next. It is also allocated beforehand, so it is ready to be used when it's needed. 
It is used when input and output to a channel happen at different speeds, like message buffers. Instead of discarding everything when the network is busy, the data is written to a ring buffer and sent out when the network is available again.
