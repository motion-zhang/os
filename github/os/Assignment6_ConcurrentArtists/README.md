...
- Did you collaborate with any other students/TAs/Professors? Not yet
- Did you use any external resources? (Cite them below)
  - slides and links provided
- (Optional) What was your favorite part of the assignment?   part1-3
- (Optional) How would you improve the assignment?            doing more readings and learning
...
You may download [xquartz](https://www.xquartz.org) which has an X11 server running. When you perform `ssh -Y username@login.ccs.neu.edu`, the 'display canvas.ppm' will magically work! (It's not really magic, because "There is no magic in Systems".)
...
>
> You: "Did you not enjoy working so efficiently(without the use of locks) without intefering with the other 64 artists?"
>
...
Michelangelo, Raphael, Donatello, and Leonardo are amongst the great artists who have been commissioned to paint on a shared 256x256 canvas. The canvas itself starts with RGB values of (255,255,255) on each pixel.  Read on for more specifications:
...
An artist moves around to adjacent and diagonal pixels. This is done through randomly selecting where the next move will be after trying to paint.
```c
...
Creating a new artist is done using malloc and populating their attributes. Each artist also has a separate thread id.
```c
...
**Discuss(in a few sentences)** how starvation may occur in the above program, and how you would possibly fix it? (You may fix it in your implementation if you like--the art may be more beautiful!)

The starvation occur when one or more threads are trying to access some data while the data is being accessed by other threads. In this context, it's like the artist is trying to draw on the pixels that are currently drawing by others which will cause the forever waiting of the threads. So when the artist want to draw continuously, but all the postion is blocked by other threads' drawing, the artist cant draw more, making the starvation.
...
I kind of missed the concepts on Virtual memory and allocators and I did not get time to do the assignment. That was the week of mid-term and I took two midterms in a roll and did another CS assignment. Assignment5 was due on Monday, but when I finally got time to do it, it was already Saturday. So I think I missed a lot of concepts in this chapter and I will try to catch up.

New things learned:
Basic concepts: 
MMU: figure out the mappings from main memory to virtual address for a process.
Virtual memory pros: Memory efficiency, simplifies memory management, isolates address spaces.
Page table: keeps track of mapping between virtual and physical addresses.
Page fault: page that is not in the memory
Steps of address translation
Dynamic memory allocation: request virtual memory during runtime, malloc as an example, using heap
Explicit Allocator: allocates and free spaces
Implicit Allocator: allocates but not free spaces

C memory functions:
calloc, realloc, free, sbrk, mmap
